...
/** Five-petal plum blossom with white heart and gold stamens. */
const plum = (cx: number, cy: number, r: number, fill: string): string => {
  let s = "";
  for (let i = 0; i < 5; i++) {
    const a = ((-90 + i * 72) * Math.PI) / 180;
    s += `<circle cx="${(cx + r * Math.cos(a)).toFixed(1)}" cy="${(cy + r * Math.sin(a)).toFixed(1)}" r="${(r * 0.62).toFixed(1)}" fill="${fill}"/>`;
  }
  s += `<circle cx="${cx}" cy="${cy}" r="${(r * 0.42).toFixed(1)}" fill="#faf4e6"/>`;
  for (let i = 0; i < 5; i++) {
    const a = ((-54 + i * 72) * Math.PI) / 180;
    s += `<circle cx="${(cx + r * 0.3 * Math.cos(a)).toFixed(1)}" cy="${(cy + r * 0.3 * Math.sin(a)).toFixed(1)}" r="1.5" fill="#d9a441"/>`;
  }
  return s;
};
...
const bud = (cx: number, cy: number, r: number): string =>
  `<circle cx="${cx}" cy="${cy}" r="${r}" fill="#9e2a20"/>` +
...
<path d="M118 58 Q150 54 170 62 Q150 72 126 68 Z" fill="#d9a441"/>
...
<path d="M14 164 Q40 156 62 142" stroke="#8a5a33" stroke-width="2" fill="none" stroke-linecap="round"/>
${plum(34, 146, 12, "#9e2a20")}
${plum(66, 200, 14, "#c8362b")}
${plum(150, 60, 12, "#c8362b")}
${plum(172, 166, 13, "#c8362b")}
${plum(58, 118, 10, "#9e2a20")}
${bud(84, 132, 5)}
${bud(160, 132, 5)}
${bud(46, 216, 4.5)}
<path d="M84 100 L64 112 L68 98 L86 92 Z" fill="#2e5d3f" stroke="#1b1b1b" stroke-width="1.5"/>
<path d="M84 98 Q88 76 112 74 Q136 72 140 90 Q142 106 118 112 Q94 118 84 98 Z" fill="#47804f" stroke="#1b1b1b" stroke-width="2.5"/>
<path d="M90 102 Q106 114 122 108 Q106 118 92 108 Z" fill="#ecd9a0"/>
<path d="M94 88 Q108 80 122 86 Q112 98 96 96 Z" fill="#2e5d3f" stroke="#1b1b1b" stroke-width="1.5"/>
<circle cx="134" cy="80" r="10" fill="#47804f" stroke="#1b1b1b" stroke-width="2.5"/>
<path d="M143 77 L157 80 L143 84 Z" fill="#d9a441" stroke="#1b1b1b" stroke-width="1.2"/>
<circle cx="136" cy="78" r="1.9" fill="#1b1b1b"/>
<g stroke="#1b1b1b" stroke-width="2.5" stroke-linecap="round" fill="none">
...
${plum(74, 172, 13, "#c8362b")}
${plum(34, 194, 11, "#9e2a20")}
${plum(52, 262, 12, "#c8362b")}
${bud(56, 200, 5)}
${bud(30, 246, 4.5)}
...
<g transform="rotate(5 121 160)">
  ...
  <rect x="102" y="66" width="38" height="190" fill="#c8362b"/>
...
${plum(126, 84, 14, "#c8362b")}
${plum(158, 100, 12, "#9e2a20")}
${plum(120, 186, 14, "#c8362b")}
${plum(42, 158, 12, "#c8362b")}
${plum(70, 244, 11, "#9e2a20")}
${bud(94, 170, 5)}
${bud(146, 116, 4.5)}
${bud(56, 130, 4.5)}
<path d="M120 40 Q150 32 178 40 Q162 52 134 50 Q112 48 120 40 Z" fill="#ecd9a0"/>
<path d="M116 262 Q146 256 172 262 Q154 272 128 270 Z" fill="#d9a441"/>
`;
...
${plum(54, 126, 14, "#c8362b")}
${plum(148, 232, 13, "#c8362b")}
${plum(128, 62, 12, "#9e2a20")}
${plum(84, 158, 11, "#c8362b")}
${plum(40, 268, 12, "#9e2a20")}
${bud(72, 118, 5)}
${bud(118, 186, 4.5)}
${bud(160, 210, 4.5)}
<path d="M22 62 Q52 54 80 62 Q62 74 36 72 Q14 70 22 62 Z" fill="#ecd9a0"/>
<path d="M120 290 Q150 284 178 290 Q160 300 132 298 Z" fill="#d9a441"/>
`;
